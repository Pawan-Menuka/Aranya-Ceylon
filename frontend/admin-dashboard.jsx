/* Aranya Ceylon — ADMIN Dashboard: 3 layout variations + rich charts.
   Depends on admin-shell.jsx atoms and window.ADMIN. Exports AdminDashboard. */
const { useState: dbState, useMemo: dbMemo } = React;

/* ---------- market-aware series + kpis ---------- */
function useDashData(market) {
  return dbMemo(() => {
  ...
  return (
    <div className="ad-card" style={{ overflow: "hidden" }}>
      ...
      <div style={{ padding: "16px 16px 10px" }}>
        <AreaChart data={range === "7d" ? d.series.slice(-7) : d.series} height={height}
          stroke="#0F6E56" fill="#0F6E56" accent="#BA7517" />
      </div>
    </div>
  );
}
...
function MarketDonutCard() {
  const A = window.ADMIN, segs = A.MARKET_SPLIT;
...
function TopProductsCard({ compact }) {
  const A = window.ADMIN;
  return (
    <SectionCard title="Top products" pad={false}
      action={<button className="ad-btn ad-btn-ghost ad-btn-sm">View all</button>}>
...
function FulfillmentCard({ onOpen }) {
  const A = window.ADMIN;
...
function ActivityCard() {
  const A = window.ADMIN;
  const toneMap = { brand: "#0F6E56", amber: "#BA7517", red: "#C0492F", slate: "#5E7587" };
  const renderText = (t) => {
    const parts = t.split("**");
...
function AlertTile({ icon, tone, value, label, sub, onGo }) {
  const toneMap = { red: "#C0492F", amber: "#BA7517", slate: "#5E7587", brand: "#0F6E56" };
  const c = toneMap[tone];
  return (
    <button onClick={onGo} style={{ textAlign: "left", cursor: "pointer", background: "var(--ad-card)", border: "1px solid var(--ad-line)",
      borderRadius: 14, padding: 16, boxShadow: "var(--shadow-sm)", display: "flex", alignItems: "center", gap: 14, width: "100%" }}>
...
/* ============================ VARIATION A — COMMAND ============================ */
function DashCommand({ d, market, setMarket, range, setRange, go }) {
  return (
...
function DashEditorial({ d, market, setMarket, range, setRange, go }) {
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
      <div style={{ display: "grid", gridTemplateColumns: "1.7fr 1fr", gap: 18, alignItems: "stretch" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          ...
          <AlertTile icon="alert" tone="red" value={window.ADMIN.LOW_STOCK.length} label="low stock" sub="Items below threshold — restock soon" onGo={() => go("products")} />
          <AlertTile icon="handshake" tone="brand" value={window.ADMIN.WHOLESALE.filter((w) => w.status !== "approved").length} label="wholesale" sub="Applications pending review" onGo={() => go("orders")} />
...
function AreaChartLight({ data }) {
  ...
  const gid = "agl" + dbMemo(() => Math.random().toString(36).slice(2), []);
  return (
...
function MiniStat({ label, value, delta, deltaInvert, bar }) {
  return (
...
function DashDense({ d, market, setMarket, range, setRange, go }) {
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(6,1fr)", gap: 12 }}>
        ...
        <MiniStat label="Low stock" value={window.ADMIN.LOW_STOCK.length} bar="#C0492F" />
...
function AdminDashboard({ go }) {
  const [layout, setLayout] = dbState(() => localStorage.getItem("ad_dash_layout") || "command");
  const [market, setMarket] = dbState("all");
  const [range, setRange] = dbState("30d");
  const d = useDashData(market);
  const setL = (k) => { setLayout(k); localStorage.setItem("ad_dash_layout", k); };
