/* Aranya Ceylon — ADMIN sample dataset.
   Fabricated back-office data for the admin prototype. Reuses window.SPICES
   (spice-data.js) and window.JOURNAL (journal-data.js) where possible.
   Exposes window.ADMIN. */
(function () {
  const S = window.SPICES || [];
  const usd = (n) => "$" + n.toLocaleString("en-US", { minimumFractionDigits: 2, maximumFractionDigits: 2 });
  const lkr = (n) => "Rs " + Math.round(n).toLocaleString("en-US");

  /* ---------- 30-day revenue series (USD-normalised totals) ---------- */
  // gentle upward trend with weekly rhythm + noise; deterministic.
  const days = [];
  ...
  const CITY_INTL = [["London", "GB"], ["New York", "US"], ["Berlin", "DE"], ["Toronto", "CA"], ["Sydney", "AU"], ["Singapore", "SG"], ["Dubai", "AE"], ["Paris", "FR"], ["Amsterdam", "NL"], ["Tokyo", "JP"]];
  const CITY_LK = [["Colombo", "LK"], ["Kandy", "LK"], ["Galle", "LK"], ["Negombo", "LK"], ["Jaffna", "LK"]];
  const STATUSES = ["paid", "processing", "shipped", "delivered", "refunded", "cancelled"];
  const PAY = { intl: "Stripe", local: "PayHere" };

  function mkItems(n, market) {
    const items = [];
    const used = new Set();
    let units = 0, sub = 0;
    for (let i = 0; i < n; i++) {
  ...
  let onum = 4821;
  for (let i = 0; i < 46; i++) {
    const market = rnd() < 0.62 ? "intl" : "local";
    const nItems = 1 + Math.floor(rnd() * 4);
    const { items, units, sub } = mkItems(nItems, market);
    const shipUsd = market === "intl" ? (sub > 60 ? 0 : 9.5) : (sub > 25 ? 0 : 2.2);
    const totalUsd = sub + shipUsd;
    // status weighted: more recent -> earlier in pipeline
    ...
    const d = new Date(today); d.setDate(today.getDate() - ageDays); d.setHours(8 + Math.floor(rnd() * 11), Math.floor(rnd() * 60));
  ...
  const PRODUCTS = S.map((sp, i) => {
    const stockEach = [4, 7, 240, 12, 410, 188][i] != null ? [4, 7, 240, 12, 410, 188][i] : 120;
    ...
    return {
      name: sp.name, latin: sp.latin, slug: sp.name.toLowerCase().replace(/\s+/g, "-"),
      sku: "AC-" + sp.name.split(" ").map((w) => w[0]).join("").toUpperCase(),
      category: cat, color: sp.color, base: sp.base, deep: sp.deep, surface: sp.surface,
      usd: sp.usd, lkr: sp.lkr, weights: sp.weights, rating: sp.rating, reviews: sp.reviews,
      badge: sp.badge,
      stock: stockEach,
      status: stockEach === 0 ? "Out of stock" : stockEach < 20 ? "Low stock" : "Active",
      sold30: TOP_PRODUCTS[i] ? TOP_PRODUCTS[i].units : 200 + i * 30,
      visible: i !== 5 ? true : true,
      featured: i < 2,
    };
  });
  // add a couple of gift sets / variants for richness
  PRODUCTS.push({
  ...
  const BLOG = J.map((p, i) => ({
    slug: p.slug, title: p.title, category: p.category, author: p.author, role: p.role,
    date: p.date, readTime: p.readTime, accent: p.accent, featured: p.featured,
    status: i < 5 ? "Published" : "Draft",
    views: [4820, 3110, 5640, 2280, 1890, 3470, 2050][i] || 900,
  }));
  // one scheduled post
  BLOG.unshift({
