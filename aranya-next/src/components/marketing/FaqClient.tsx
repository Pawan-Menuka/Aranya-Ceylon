...
interface FaqCat { id: string; name: string; icon: string; items: [string, string][]; }

const FAQ_CATS: FaqCat[] = [
  {
...
function FAQRail({ active, onJump }: { active: string; onJump: (id: string) => void }) {
  return (
    <nav style={{ position: "sticky", top: 130, display: "flex", flexDirection: "column", gap: 4 }}>
      ...
      {FAQ_CATS.map((c) => {
        ...
        return (
          <button key={c.id} onClick={() => onJump(c.id)} style={{ display: "flex", alignItems: "center", gap: 12, padding: "12px 14px", borderRadius: 11, cursor: "pointer", textAlign: "left", background: on ? "var(--surface)" : "transparent", border: on ? "1px solid var(--line)" : "1px solid transparent", transition: "background .15s" }}>
...
function FAQBody() {
  ...
  const [open, setOpen] = React.useState("orders-0");
...
export function FaqClient() {
  ...
  return (
    <div data-screen-label="FAQ">
      <SupportHeader active="FAQ" eyebrow="Support · FAQ"
        title={<>Questions,<br />answered.</>}
        lead="Everything about orders, shipping, our spices, payments and returns — grouped so you can find it fast." />
      <FAQBody />
