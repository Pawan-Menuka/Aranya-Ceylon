...
// Support cluster shared parts (ported from support-common.jsx). Dark branded
// header with the three-tab cluster nav (Contact · FAQ · Shipping & Returns)
// and a stroke icon set (SIcon). The Contact page is built; FAQ/Shipping routes
// resolve to 404 until ported (wired & ready, like other footer links).

export function SIcon({ name, size = 22, stroke = "var(--brand)", w = 1.6 }: { name: string; size?: number; stroke?: string; w?: number }) {
...
function SupportTabs({ active }: { active: string }) {
  const tabs: [string, string][] = [["Contact", "/contact"], ["FAQ", "/faq"], ["Shipping & Returns", "/shipping"]];
  return (
...
export function SupportHeader({ active, title, lead, eyebrow = "Support" }: { active: string; title: React.ReactNode; lead?: string; eyebrow?: string }) {
  return (
...
// Reusable "still need help" CTA band (ported from support-common.jsx).
export function SupportCTA({ market = "intl", exclude }: { market?: Market; exclude?: string }) {
      ...
      <div className="home-section-pad" style={{ position: "relative", maxWidth: 760, margin: "0 auto", padding: "0 40px", textAlign: "center" }}>
        ...
        <Reveal delay={150}>
          <div style={{ display: "flex", gap: 13, justifyContent: "center", flexWrap: "wrap" }}>
            {exclude !== "contact" && <Link href="/contact" className={btn} style={{ width: "auto", padding: "14px 28px", display: "inline-block" }}>Contact us</Link>}
