...
// (domestic LKR / international USD), tracking explainer, customs note, returns.
...
function RateCard({ region, you, currencyNote, freeLine, rows, market }: RateCardProps) {
    ...
    <section style={{ background: "var(--bg)", padding: "76px 0 90px" }}>
      <div className="home-section-pad" style={{ maxWidth: 1080, margin: "0 auto", padding: "0 40px" }}>
        ...
        <div className="ship-rates" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 22, alignItems: "stretch" }}>
          <Reveal style={{ display: "flex" }}><div style={{ flex: 1, display: "flex" }}><RateCard {...domestic} /></div></Reveal>
          <Reveal delay={80} style={{ display: "flex" }}><div style={{ flex: 1, display: "flex" }}><RateCard {...intl} /></div></Reveal>
        </div>
        <Reveal delay={120}>
    ...
    <section style={{ background: "var(--surface)", padding: "92px 0", borderTop: "1px solid var(--line)", borderBottom: "1px solid var(--line)" }}>
      <div className="home-section-pad" style={{ maxWidth: 1100, margin: "0 auto", padding: "0 40px" }}>
        <div style={{ display: "flex", alignItems: "flex-end", justifyContent: "space-between", gap: 20, flexWrap: "wrap", marginBottom: 44 }}>
          ...
          <Reveal delay={100}><Link href="/account" className="btn btn-ghost" style={{ width: "auto", padding: "12px 22px", display: "inline-flex", alignItems: "center", gap: 8 }}><SIcon name="truck" size={17} stroke="var(--brand)" />Track my order</Link></Reveal>
        </div>
        ...
        <Reveal delay={120}>
          <div style={{ display: "flex", gap: 14, alignItems: "flex-start", marginTop: 40, padding: "20px 22px", background: "#fff", border: "1px solid var(--line)", borderRadius: 13, maxWidth: 760 }}>
            ...
            <p style={{ fontFamily: "var(--font-ui)", fontSize: 13.5, color: "var(--muted)", margin: 0, lineHeight: 1.65 }}>
              <b style={{ color: "var(--ink)" }}>International duties &amp; taxes.</b> Customs charges are set by the destination country and are the recipient&rsquo;s responsibility — they&rsquo;re not included in our prices or shipping. Most personal-volume orders clear without issue.
            </p>
          </div>
        </Reveal>
      </div>
...
function ReturnsPolicy({ market }: { market: Market }) {
      ...
      <div className="home-section-pad" style={{ maxWidth: 1080, margin: "0 auto", padding: "0 40px" }}>
        <div style={{ textAlign: "center", marginBottom: 46 }}>
          ...
          <Reveal delay={100}><p style={{ fontFamily: "var(--font-ui)", fontSize: 14.5, color: "var(--muted)", margin: "14px auto 0", maxWidth: 540, lineHeight: 1.6 }}>
            Spice is a fresh, consumable product, so opened items can&rsquo;t be resold — but if the quality ever falls short, we&rsquo;ll always make it right.
          </p></Reveal>
        </div>
          ...
          <div style={{ background: "#1A1A1A", color: "#FDFAF5", borderRadius: 18, padding: "clamp(28px,3vw,44px)", position: "relative", overflow: "hidden" }}>
            ...
            <div style={{ position: "relative" }}>
              ...
              <div style={{ marginTop: 34, paddingTop: 24, borderTop: "1px solid rgba(253,250,245,.16)", display: "flex", gap: 13, flexWrap: "wrap" }}>
                <Link href="/contact" className={local ? "btn btn-local" : "btn btn-intl"} style={{ width: "auto", padding: "13px 26px", display: "inline-block" }}>Start a return</Link>
...
export function ShippingClient() {
  ...
  return (
    <div data-screen-label="Shipping & Returns">
      <SupportHeader active="Shipping & Returns" eyebrow="Support · Shipping & Returns"
        title={<>Shipping &amp;<br />returns.</>}
        lead="Rates, delivery times and our fair returns policy — for orders within Sri Lanka and to 40+ countries worldwide." />
      <ShipRates market={market} />
      <ShipTracking />
      <ReturnsPolicy market={market} />
      <SupportCTA market={market} />
    </div>
  );
}
