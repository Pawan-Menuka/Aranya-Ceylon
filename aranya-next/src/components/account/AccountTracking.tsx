...
// Order-tracking UI (ported from account-tracking.jsx):
// StatusPill · CopyTracking · TrackingSummary · TrackingTimeline · OrderLine · OrderDetailView.

function tone(t: "forest" | "amber" | "muted") {
  if (t === "forest") return { fg: "#0F6E56", bg: "rgba(15,110,86,.10)", dot: "#1D9E75", line: "rgba(15,110,86,.22)" };
  if (t === "amber") return { fg: "#9A5E11", bg: "rgba(186,117,23,.12)", dot: "#BA7517", line: "rgba(186,117,23,.24)" };
  return { fg: "#5C5248", bg: "rgba(92,82,72,.10)", dot: "#8A7F71", line: "rgba(92,82,72,.20)" };
}
...
export function StatusPill({ status, size = "md" }: { status: string; size?: "sm" | "md" }) {
  ...
  return (
    <span style={{ display: "inline-flex", alignItems: "center", gap: 7, background: t.bg, color: t.fg, borderRadius: 999, padding: pad, fontFamily: "var(--font-ui)", fontSize: fs, fontWeight: 700, letterSpacing: ".03em", lineHeight: 1, whiteSpace: "nowrap" }}>
...
function CopyTracking({ value, carrier }: { value: string; carrier: string }) {
  const [copied, setCopied] = React.useState(false);
  return (
    <button onClick={() => { try { navigator.clipboard.writeText(value); } catch { /* ignore */ } setCopied(true); setTimeout(() => setCopied(false), 1400); }} style={{ display: "inline-flex", alignItems: "center", gap: 9, background: "#fff", border: "1px solid var(--line)", borderRadius: 8, padding: "9px 12px", cursor: "pointer", fontFamily: "var(--font-ui)", textAlign: "left" }}>
...
export function TrackingSummary({ order, market, onView }: { order: AccountOrder; market: Market; onView: () => void }) {
  ...
  const headline = order.status === "delivered" ? "Delivered " + (order.deliveredLabel || "") : order.status === "out_for_delivery" ? "Arriving today" : order.status === "processing" ? "Being prepared" : "Arriving " + (order.etaLabel || "soon");
  return (
    <div style={{ background: "var(--brand)", color: "#FDFAF5", borderRadius: 14, overflow: "hidden", position: "relative" }}>
      ...
      <div style={{ position: "relative", padding: "26px 30px 30px" }}>
              ...
              return (
                <div key={s.key} style={{ display: "flex", flexDirection: "column", alignItems: "center", flex: 1, minWidth: 0 }}>
                  <span style={{ position: "relative", width: 18, height: 18, display: "grid", placeItems: "center" }}>
                    <span style={{ width: done || active ? 18 : 11, height: done || active ? 18 : 11, borderRadius: 999, background: done ? "#E6B860" : active ? "#FDFAF5" : "rgba(253,250,245,.28)", border: active ? "2px solid #E6B860" : "none", display: "grid", placeItems: "center", transition: "all .3s" }}>
                  ...
                  <span style={{ fontFamily: "var(--font-ui)", fontSize: 10.5, fontWeight: 600, marginTop: 9, textAlign: "center", color: done || active ? "rgba(253,250,245,.95)" : "rgba(253,250,245,.5)", lineHeight: 1.25, letterSpacing: ".01em" }}>{s.label}</span>
                </div>
              );
            })}
          </div>
        ...
        <div style={{ marginTop: 26, display: "flex", alignItems: "center", justifyContent: "space-between", gap: 16, flexWrap: "wrap" }}>
          ...
          <button onClick={onView} style={{ display: "inline-flex", alignItems: "center", gap: 8, background: "#E6B860", color: "#1A1A1A", border: 0, borderRadius: 8, padding: "12px 18px", cursor: "pointer", fontFamily: "var(--font-ui)", fontSize: 13.5, fontWeight: 700 }}>
            View full tracking
  ...
  return (
    <div style={{ position: "relative", paddingLeft: 4 }}>
      {tl.map((s, i) => {
        ...
        const dotBg = done ? "var(--brand)" : "#fff";
          ...
          <div key={s.key} style={{ display: "flex", gap: 18, position: "relative" }}>
            <div style={{ display: "flex", flexDirection: "column", alignItems: "center", flex: "0 0 auto" }}>
              <span style={{ position: "relative", width: 26, height: 26, display: "grid", placeItems: "center", flex: "0 0 auto", marginTop: 1 }}>
                ...
                <span style={{ position: "relative", width: 26, height: 26, borderRadius: 999, background: dotBg, border: "2px solid " + dotBorder, display: "grid", placeItems: "center", boxSizing: "border-box" }}>
            ...
            <div style={{ paddingBottom: last ? 0 : 26, flex: 1, minWidth: 0, marginTop: 1 }}>
              ...
              {s.loc && (
                <div style={{ display: "inline-flex", alignItems: "center", gap: 6, marginTop: 8, fontFamily: "var(--font-ui)", fontSize: 12.5, fontWeight: 600, color: "var(--brand)" }}>
...
export function OrderDetailView({ order, market, address, onBack, onReorder }: { order: AccountOrder; market: Market; address: AccountAddress | null; onBack: () => void; onReorder: (o: AccountOrder) => void }) {
  ...
  return (
    <div>
      <button onClick={onBack} style={{ display: "inline-flex", alignItems: "center", gap: 7, background: "none", border: 0, cursor: "pointer", fontFamily: "var(--font-ui)", fontSize: 13, fontWeight: 700, color: "var(--muted)", padding: 0, marginBottom: 20 }}>
