...
// Categories landing (ported from categories.jsx). Deep-links into the catalog
// via /products?cat= / ?flavour= / ?sort=.
...
const COLLECTIONS = [
  { name: "Bestsellers", href: "/products?sort=best", color: "#BA7517", deep: "#8A560F", slot: "col-best", note: "What the kitchen reaches for" },
  { name: "New Arrivals", href: "/products?sort=new", color: "#1D9E75", deep: "#147a59", slot: "col-new", note: "Fresh to the harvest table" },
  { name: "Gift Sets", href: "/gifts", color: "#0F6E56", deep: "#0b5343", slot: "col-gift", note: "Curated wooden boxes" },
];

function ArrowBtn({ hovered, size = 44 }: { hovered: boolean; size?: number }) {
  return (
    <span style={{ flex: "0 0 auto", width: size, height: size, borderRadius: 999, background: hovered ? "var(--accent)" : "rgba(253,250,245,.16)", border: "1px solid rgba(253,250,245,.4)", backdropFilter: "blur(3px)", display: "grid", placeItems: "center", transition: "background .25s, transform .25s", transform: hovered ? "translateX(3px)" : "none" }}>
      ...
      <div style={{ maxWidth: 1280, margin: "0 auto", padding: "0 40px" }}>
        <div className="cg-feature" style={{ display: "grid", gridTemplateColumns: "minmax(0,5fr) minmax(0,6fr)", gap: 60, alignItems: "center", direction: flip ? "rtl" : "ltr" }}>
          <Reveal style={{ direction: "ltr" }}>
            <Link href={href} onMouseEnter={() => setH(true)} onMouseLeave={() => setH(false)} style={{ position: "relative", display: "block", borderRadius: 12, overflow: "hidden", aspectRatio: "4 / 3", boxShadow: h ? "var(--shadow-lg)" : "var(--shadow-md)", transition: "box-shadow .3s" }}>
              <ImageSlot id={p.slot} shape="rect" fit="cover" placeholder={`Drop a ${title} photo`} style={{ position: "absolute", inset: 0, width: "100%", height: "100%", display: "block" }} />
          ...
          <div style={{ direction: "ltr" }}>
            ...
            <Reveal delay={60}><h2 className="disp" style={{ fontSize: "clamp(34px,3.8vw,52px)", color: "var(--brand)", margin: "14px 0 18px", lineHeight: 1.04 }}>{title}</h2></Reveal>
            <Reveal delay={110}><p className="prose" style={{ fontSize: 17.5, color: "var(--ink)", margin: "0 0 26px", maxWidth: 520 }}>{p.blurb}</p></Reveal>
            <Reveal delay={150}>
              <Link href={href} className={market === "local" ? "btn btn-local" : "btn btn-intl"} style={{ width: "auto", padding: "13px 28px", display: "inline-flex", textDecoration: "none" }}>
...
function FlavourCard({ f }: { f: (typeof FLAVOURS)[number] }) {
  const [h, setH] = React.useState(false);
  return (
    <Link href={"/products?flavour=" + encodeURIComponent(f.name)} onMouseEnter={() => setH(true)} onMouseLeave={() => setH(false)} style={{ display: "flex", alignItems: "center", gap: 16, textDecoration: "none", background: "rgba(253,250,245,.06)", border: "1px solid rgba(253,250,245,.16)", borderRadius: 12, padding: "20px 22px", transition: "background .2s, transform .2s", transform: h ? "translateY(-2px)" : "none" }}>
    ...
    <section style={{ background: "var(--brand)", color: "#FDFAF5", padding: "96px 0", position: "relative", overflow: "hidden" }}>
      ...
      <div style={{ maxWidth: 1180, margin: "0 auto", padding: "0 40px", position: "relative" }}>
        <div style={{ textAlign: "center", marginBottom: 50 }}>
          ...
          <Reveal delay={110}><p className="prose" style={{ fontSize: 17, color: "rgba(253,250,245,.82)", margin: "0 auto", maxWidth: 520 }}>Cook to a feeling, not a name — pick the note you want on the plate and we&rsquo;ll show you the spices that carry it.</p></Reveal>
        </div>
        <Reveal delay={120}>
...
function CollectionTile({ c }: { c: (typeof COLLECTIONS)[number] }) {
  const [h, setH] = React.useState(false);
  return (
    <Link href={c.href} onMouseEnter={() => setH(true)} onMouseLeave={() => setH(false)} style={{ position: "relative", display: "block", borderRadius: 12, overflow: "hidden", aspectRatio: "5 / 4", boxShadow: h ? "var(--shadow-lg)" : "var(--shadow-sm)", transition: "box-shadow .3s" }}>
      <ImageSlot id={c.slot} shape="rect" fit="cover" placeholder={`Drop a ${c.name} photo`} style={{ position: "absolute", inset: 0, width: "100%", height: "100%", display: "block" }} />
    ...
    <div data-screen-label="Categories">
      <header style={{ background: "var(--brand)", color: "#FDFAF5", position: "relative", overflow: "hidden" }}>
        ...
        <div style={{ maxWidth: 1280, margin: "0 auto", padding: "132px 40px 56px", position: "relative", textAlign: "center" }}>
          ...
          <Reveal delay={100}><h1 className="disp" style={{ fontSize: "clamp(46px,6vw,82px)", lineHeight: 1.0, margin: "16px 0 14px", fontWeight: 600 }}>Shop by category</h1></Reveal>
          <Reveal delay={150}><p className="prose" style={{ fontSize: 18, color: "rgba(253,250,245,.8)", margin: "0 auto", maxWidth: 540 }}>Whole spices, stone-milled powders and estate blends — or browse by the flavour you&rsquo;re cooking toward.</p></Reveal>
        </div>
      </header>
      ...
      <section style={{ background: "var(--bg)", padding: "60px 0 104px", textAlign: "center" }}>
        <Reveal>
          <Link href="/products" className="btn btn-ghost" style={{ width: "auto", padding: "15px 36px", display: "inline-flex", textDecoration: "none" }}>
