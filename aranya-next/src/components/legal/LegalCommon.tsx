...
import { SIcon } from "../marketing/SupportCommon";
import { useMarket } from "../MarketContext";

// Legal cluster shared parts (ported from legal-common.jsx). Dark branded header
// with a three-tab cluster nav (Privacy · Terms · Cookies), and a two-column
// body: sticky table-of-contents sidebar + long-form .prose content (Spectral).

export interface LegalSection { id: string; title: string; body: React.ReactNode; }

function LegalTabs({ active }: { active: string }) {
  const tabs: [string, string][] = [["Privacy", "/privacy"], ["Terms", "/terms"], ["Cookies", "/cookies"]];
  return (
    <div style={{ display: "inline-flex", flexWrap: "wrap", justifyContent: "center", gap: 8, background: "rgba(253,250,245,.06)", border: "1px solid rgba(253,250,245,.16)", borderRadius: 999, padding: 5 }}>
      {tabs.map(([label, href]) => {
        ...
        return (
          <Link key={label} href={href} style={{ fontFamily: "var(--font-ui)", fontSize: 13.5, fontWeight: 700, letterSpacing: ".01em", padding: "10px 22px", borderRadius: 999, whiteSpace: "nowrap", transition: "background .18s, color .18s", background: on ? "#E6B860" : "transparent", color: on ? "#1A1A1A" : "rgba(253,250,245,.82)" }}
...
export function LP({ children, lead }: { children: React.ReactNode; lead?: boolean }) {
...
export function LCallout({ icon = "shield", children }: { icon?: string; children: React.ReactNode }) {
  return (
...
function LegalLayout({ sections }: { sections: LegalSection[] }) {
        ...
        <aside className="legal-toc" style={{ position: "sticky", top: 96 }}>
          ...
          <nav style={{ display: "flex", flexDirection: "column", gap: 2, borderLeft: "1px solid var(--line)" }}>
            {sections.map((s) => {
              ...
              return (
                <a key={s.id} href={"#" + s.id} onClick={(e) => jump(e, s.id)} style={{ fontFamily: "var(--font-ui)", fontSize: 13.5, fontWeight: on ? 700 : 500, lineHeight: 1.4, color: on ? "var(--brand)" : "var(--muted)", padding: "8px 0 8px 16px", marginLeft: -1, borderLeft: on ? "2px solid var(--brand)" : "2px solid transparent", transition: "color .15s, border-color .15s" }}
          ...
          <Reveal>
            <div style={{ marginTop: 52, padding: "26px 28px", background: "#1A1A1A", color: "#FDFAF5", borderRadius: 16, position: "relative", overflow: "hidden" }}>
              ...
              <div style={{ position: "relative", display: "flex", flexWrap: "wrap", gap: 20, alignItems: "center", justifyContent: "space-between" }}>
                ...
                <Link href="/contact" className={market === "local" ? "btn btn-local" : "btn btn-intl"} style={{ width: "auto", padding: "13px 26px", display: "inline-block", whiteSpace: "nowrap" }}>Contact us</Link>
              </div>
            </div>
          </Reveal>
        </div>
...
// Full legal page shell: dark header + TOC layout. Pages pass their section data.
export function LegalClient({ active, title, lead, updated, sections }: { active: string; title: React.ReactNode; lead?: string; updated?: string; sections: LegalSection[] }) {
  return (
    <div data-screen-label={typeof active === "string" ? active : "Legal"}>
      <LegalHeader active={active} title={title} lead={lead} updated={updated} />
