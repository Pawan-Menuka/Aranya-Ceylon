...
| 10 | Add CDN | be present | present | **PASS** | Cloudinary configured for product images (`next.config.mjs:16-21`); Cloudflare documented as the recommended front door (`DEPLOYMENT_CHECKLIST.md:51-56`) with geo-detection already wired (`backend/src/middleware/market.ts:36`). |
...
| 13 | Lighthouse audit | be present | absent | **FAIL** | No Lighthouse tooling anywhere (no `.lighthouserc`, no CI step). Only an unchecked manual row in `DEPLOYMENT_CHECKLIST.md:191` ("U" = untested). |
...
| 12 | Low | repo-wide | No Lighthouse CI automation, only a manual unchecked checklist row | Added `aranya-next/lighthouserc.json` + a `Lighthouse CI` step to `.github/workflows/ci.yml`'s `frontend` job (runs after `next build`, audits the homepage). Thresholds mirror the existing manual target in `DEPLOY_READINESS_PLAN.md` item 4.7 (Perf ≥ 70, a11y ≥ 90, no CLS failures) rather than inventing new numbers. All assertions are `warn`-level plus `continue-on-error: true` — report-only until the check has a stability track record, so it can't block unrelated PRs on Lighthouse variance from a shared runner. | done |
