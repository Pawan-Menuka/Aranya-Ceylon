...
_Added 2026-07-06. Ordered by risk and dependency — the same convention as the [first audit report](audit-report.md). Each phase is independently shippable and testable. Effort is rough dev-time; Risk is the chance a fix destabilises something else. Items marked **needs runtime** can't be fully verified without a live DB/deploy._
...
Batch as time allows. Includes carry-overs from the [first audit report](audit-report.md) that remain open.
...
| **PERF-03/04/05/06/08/09/11** | Micro-optimizations carried over from the [first audit report](audit-report.md) — batch queries, parallelize waterfalls, debounce totals, memoize context. | Varies |
