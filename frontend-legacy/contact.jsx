/* Aranya Ceylon — CONTACT page sections.
   Depends on support-common.jsx (SupportHeader, SIcon, SupportCTA), home-common.jsx
   (Reveal, Liyawel, Eyebrow), image-slot.js. Exports ContactChannels, ContactMain, ContactVisit. */
...
/* ============ Quick channel cards (overlap the header) ============ */
function ContactChannels({ market }) {
  const cards = [
...
/* ============ Main: form (left) + directory (right) ============ */
function ContactMain({ market }) {
  const [done, setDone] = cState(false);
  const [ref, setRef] = cState("");
  const [f, setF] = cState({ name: "", email: "", order: "", subject: "", message: "", consent: false });
  ...
  const field = (label, key, type = "text", ph = "", req = true) => (
    <label style={{ display: "flex", flexDirection: "column", gap: 7 }}>
      ...
      <input required={req} type={type} placeholder={ph} value={f[key]} onChange={(e) => set(key, e.target.value)} className="sp-input" />
    </label>
  );
            ...
            {done ? (
              <div style={{ textAlign: "center", padding: "26px 6px" }}>
                ...
                <p className="prose" style={{ fontSize: 16, color: "var(--ink)", margin: "0 auto 8px", maxWidth: 380 }}>
                  Thank you, {f.name ? f.name.split(" ")[0] : "there"}. Your ticket reference is <b style={{ color: "var(--accent)" }}>{ref}</b>. We'll reply to {f.email || "your inbox"} within one business day.
                </p>
                <button className="btn btn-ghost" style={{ width: "auto", padding: "12px 24px", margin: "20px auto 0" }} onClick={() => { setDone(false); setF({ name: "", email: "", order: "", subject: "", message: "", consent: false }); }}>Send another</button>
              </div>
            ) : (
              <form onSubmit={submit}>
                ...
                <div className="ct-grid2" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 18 }}>
                  ...
                  <label style={{ display: "flex", flexDirection: "column", gap: 7 }}>
                    <span style={{ fontFamily: "var(--font-ui)", fontSize: 12.5, fontWeight: 700, color: "var(--ink)" }}>What's it about?<span style={{ color: "var(--accent)" }}> *</span></span>
                ...
                <button type="submit" className={btn} style={{ marginTop: 24, padding: "15px" }}>Send message</button>
              </form>
            )}
          </div>
        </Reveal>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 26 }}>
          ...
          <Reveal delay={130}>
            <div style={{ background: "var(--brand)", color: "#FDFAF5", borderRadius: 16, padding: "24px 24px" }}>
              ...
              {[["whatsapp", "WhatsApp", "Minutes, in working hours"], ["mail", "Email", "Within one business day"], ["headset", "Order issues", "Same day, Mon–Sat"]].map(([ic, t, v]) => (
          ...
          <Reveal delay={170}>
            <div>
              ...
              <div style={{ display: "flex", gap: 10 }}>
                {[["Instagram", "@aranyaceylon"], ["Facebook", "/aranyaceylon"], ["Pinterest", "/aranyaceylon"]].map(([s, h]) => (
...
/* ============ Visit us — address + map placeholder ============ */
      ...
      <div className="home-section-pad ct-visit" style={{ maxWidth: 1180, margin: "0 auto", padding: "0 40px", display: "grid", gridTemplateColumns: "0.9fr 1.1fr", gap: 52, alignItems: "center" }}>
        <div>
          ...
          <Reveal delay={110}><p className="prose" style={{ fontSize: 16.5, color: "var(--muted)", margin: "18px 0 26px", maxWidth: 420 }}>
            Our milling room and trade counter sit in the hill country, a short drive from the Temple of the Tooth. Visits are by appointment — we'll put the kettle on.
          </p></Reveal>
          <Reveal delay={150}>
            <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
              {[["pin", CONTACT.address.join(", ")], ["clock", CONTACT.hours], ["phone", CONTACT.phone]].map(([ic, t]) => (
        ...
        <Reveal delay={90}>
          <div style={{ position: "relative", borderRadius: 16, overflow: "hidden", aspectRatio: "4 / 3", boxShadow: "var(--shadow-lg)", border: "1px solid var(--line)" }}>
            <image-slot id="contact-map" shape="rect" fit="cover" placeholder="Drop a map screenshot or storefront photo of the Kandy spice house"
...
Object.assign(window, { ContactChannels, ContactMain, ContactVisit });
