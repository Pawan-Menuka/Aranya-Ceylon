/* Aranya Ceylon — ADMIN Orders: filterable list + detail drawer (status, refund).
   Depends on admin-shell.jsx + window.ADMIN. Exports AdminOrders. */
const { useState: orState, useMemo: orMemo, useEffect: orEffect } = React;

const orFmtUSD = (n) => "$" + n.toLocaleString("en-US", { minimumFractionDigits: 2, maximumFractionDigits: 2 });
const orDate = (d) => d.toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" });
const orTime = (d) => d.toLocaleTimeString("en-US", { hour: "numeric", minute: "2-digit" });
        ...
        <div className="ad-seg" style={{ flexWrap: "wrap" }}>
          {ORDER_TABS.map((t) => (
            <button key={t.key} className={tab === t.key ? "on" : ""} onClick={() => setTab(t.key)}>
              {t.label}{counts[t.key] != null && <span style={{ marginLeft: 6, opacity: .6, fontVariantNumeric: "tabular-nums" }}>{counts[t.key]}</span>}
            </button>
          ))}
        </div>
...
function OrderDrawer({ order, onClose, onStatus, onRefund }) {
  const [confirmRefund, setConfirmRefund] = orState(false);
  orEffect(() => {
      ...
      <aside className="ad-drawer">
        ...
        <div style={{ padding: "20px 24px", borderBottom: "1px solid var(--ad-line)", display: "flex", alignItems: "flex-start", gap: 14, background: "var(--ad-card)" }}>
          <div style={{ flex: 1 }}>
            ...
            <div style={{ fontSize: 12.5, color: "var(--ad-muted)", marginTop: 4 }}>{orDate(o.date)} at {orTime(o.date)} · <MarketTag market={o.market} /></div>
          </div>
                ...
                {STATUS_FLOW.map((s, i) => {
                  ...
                  return (
                    <button key={s} onClick={() => onStatus(o, s)} className="ad-btn ad-btn-sm"
                      style={{ background: active ? "var(--brand)" : passed ? "rgba(15,110,86,.1)" : "#fff", color: active ? "#fff" : passed ? "var(--pos-deep)" : "var(--ad-muted)",
                        border: active ? "0" : "1px solid var(--ad-line-2)", textTransform: "capitalize" }}>
              ...
              {tl.slice().reverse().map((s, i, arr) => (
                <div key={i} style={{ display: "flex", gap: 13 }}>
                  <div style={{ display: "flex", flexDirection: "column", alignItems: "center" }}>
                    <span style={{ width: 13, height: 13, borderRadius: 999, background: s.warn ? "var(--neg)" : i === 0 ? "var(--brand)" : "#fff", border: s.warn ? "0" : i === 0 ? "0" : "2px solid var(--brand)", marginTop: 2 }} />
          ...
          <div className="ad-card" style={{ overflow: "hidden" }}>
            ...
            <div style={{ padding: "6px 16px 12px" }}>
              ...
              <div style={{ marginTop: 12, display: "flex", flexDirection: "column", gap: 7, fontSize: 13 }}>
                ...
                <Row k={<b style={{ fontWeight: 800 }}>Total</b>} v={<b className="tnum" style={{ fontWeight: 800, fontSize: 15 }}>{orFmtUSD(o.totalUsd)}</b>} big />
              </div>
            </div>
          </div>
...
function AdminOrders({ initialOpen }) {
  const [tab, setTab] = orState("all");
  const [market, setMarket] = orState("all");
  const [q, setQ] = orState("");
  const [orders, setOrders] = orState(() => window.ADMIN.ORDERS.map((o) => ({ ...o })));
  const [open, setOpen] = orState(null);
  ...
  const filtered = orMemo(() => orders.filter((o) => {
  ...
  const refund = (o) => setStatus(o, "refunded");

  const pendingCount = counts.paid + counts.processing;
    ...
    <div>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: 16, fontSize: 12.5, color: "var(--ad-faint)" }}>
        ...
        <div style={{ display: "flex", gap: 6 }}>
          <button className="ad-btn ad-btn-ghost ad-btn-sm" disabled style={{ opacity: .5 }}><AIcon name="chevronL" size={14} stroke="var(--ad-muted)" />Prev</button>
