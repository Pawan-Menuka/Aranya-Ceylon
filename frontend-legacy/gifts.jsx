/* Aranya Ceylon — GIFT SETS page sections.
   Curated bundles from gifts-data.js (GIFTS, GIFT_OCCASIONS), priced from CATALOG.
   GiftBox is a styled top-down "ribboned box" placeholder (kin to SpicePhoto) so the page
   reads finished before real photography. Adds sets to the shared cart as a single line.
   Depends on home-common.jsx (Reveal, Liyawel, Eyebrow), shared.jsx (Seal), image-slot.js.
   Exports GiftHero, FeaturedSet, GiftGrid, FinishingBand, GiftOccasions, GiftCorporate. */
const { useState: gUse } = React;

/* ---------------- helpers ---------------- */
function gCat(name) { return (window.CATALOG || []).find((p) => p.name === name); }
function gNum(p) { return parseFloat(String(p).replace(/[^0-9.]/g, "")); }
const GIFT_JAR_MULT = { "50g": 0.6, "100g": 1, "250g": 2.3 };
function gPrice(set, market) { return market === "local" ? set.lkr : set.usd; }
function gFmt(n, market) { return market === "local" ? "Rs " + Math.round(n).toLocaleString("en-US") : "$" + n.toFixed(2); }
function gAlaCarte(set, market) {
  const mult = GIFT_JAR_MULT[set.jar] || 1;
  let sum = 0;
  set.contents.forEach((nm) => { const p = gCat(nm); if (p) sum += gNum(market === "local" ? p.lkr : p.usd) * mult; });
  return sum;
}
function gSavePct(set, market) {
  const alc = gAlaCarte(set, market), price = gNum(gPrice(set, market));
  if (!alc || alc <= price) return 0;
  return Math.round((1 - price / alc) * 100);
}
function gAdd(set) {
  if (window.AranyaCart) window.AranyaCart.add(set, "Gift box", "Set");
  if (window.__openCart) window.__openCart();
}

/* ---------------- local icon set ---------------- */
function GIcon({ name, size = 22, stroke = "var(--brand)", w = 1.6 }) {
  const p = {
...
/* ---------------- styled "ribboned box" placeholder ---------------- */
function GiftBox({ set, ratio = "4 / 3" }) {
  const dots = set.contents.map((nm) => (gCat(nm) || {}).color || set.color);
  return (
    <div className="grain" style={{
      ...
      {/* horizontal ribbon */}
      <div style={{ position: "absolute", left: 0, right: 0, top: "calc(50% - 21px)", height: 42,
        background: `linear-gradient(180deg, ${set.base}, ${set.color} 55%, ${set.deep})`, boxShadow: "0 4px 14px rgba(0,0,0,.16)" }}>
        <span style={{ position: "absolute", left: 0, right: 0, top: "50%", height: 1.4, background: "rgba(230,184,96,.7)", transform: "translateY(-50%)" }} />
      </div>
      {/* vertical ribbon */}
      <div style={{ position: "absolute", top: 0, bottom: 0, left: "calc(50% - 25px)", width: 50,
        background: `linear-gradient(90deg, ${set.base}, ${set.color} 55%, ${set.deep})`, boxShadow: "0 0 16px rgba(0,0,0,.14)" }}>
        <span style={{ position: "absolute", top: 0, bottom: 0, left: "50%", width: 1.4, background: "rgba(230,184,96,.7)", transform: "translateX(-50%)" }} />
      </div>
      {/* wax seal */}
      <div style={{ position: "absolute", left: "50%", top: "50%", transform: "translate(-50%,-50%)", width: 58, height: 58, borderRadius: 999,
...
/* contents list (dot + name) */
function GiftContents({ set, columns = 2 }) {
  return (
    <div style={{ display: "grid", gridTemplateColumns: `repeat(${columns}, minmax(0,1fr))`, gap: "8px 16px" }}>
      {set.contents.map((nm) => {
        const c = (gCat(nm) || {}).color || set.color;
        return (
...
/* price block (big set price + à-la-carte strike + save pill) */
function GiftPrice({ set, market, size = 30 }) {
  const alc = gAlaCarte(set, market), save = gSavePct(set, market);
  return (
    <div style={{ display: "flex", alignItems: "baseline", gap: 12, flexWrap: "wrap" }}>
      <span className="disp" style={{ fontSize: size, color: "var(--ink)", fontWeight: 600, lineHeight: 1 }}>{gPrice(set, market)}</span>
      {save > 0 && <span style={{ fontFamily: "var(--font-ui)", fontSize: 13.5, color: "var(--muted)", textDecoration: "line-through" }}>{gFmt(alc, market)}</span>}
...
/* ======================= HERO ======================= */
function GiftHero({ market, onShop }) {
  ...
  return (
    <header data-hero style={{ position: "relative", minHeight: "90vh", background: "#161412", color: "#FDFAF5", overflow: "hidden", display: "flex", alignItems: "flex-end" }}>
      <image-slot id="gift-hero" shape="rect" fit="cover" placeholder="Drop a gifting photo — ribboned spice boxes, a wrapped gift being handed over"
      ...
      <div className="home-section-pad" style={{ position: "relative", maxWidth: 1280, margin: "0 auto", padding: "0 40px 84px", width: "100%" }}>
        ...
        <Reveal delay={270}>
          <div style={{ display: "flex", gap: 26, marginTop: 46, flexWrap: "wrap", borderTop: "1px solid rgba(253,250,245,.18)", paddingTop: 22, maxWidth: 640 }}>
            {[["gift", "Complimentary gift wrap"], ["pen", "Handwritten note card"], ["plane", "Ships worldwide"]].map(([ic, t]) => (
...
function FeaturedSet({ market, gridRef }) {
  const set = (window.GIFTS || []).find((g) => g.featured) || window.GIFTS[0];
  const [added, setAdded] = gUse(false);
  const onAdd = () => { gAdd(set); setAdded(true); setTimeout(() => setAdded(false), 1300); };
  const btn = market === "local" ? "btn btn-local" : "btn btn-intl";
  return (
      ...
      <div className="home-section-pad gift-feature" style={{ maxWidth: 1180, margin: "0 auto", padding: "0 40px", display: "grid", gridTemplateColumns: "1.05fr 0.95fr", gap: 60, alignItems: "center" }}>
        <Reveal>
          <div style={{ position: "relative", borderRadius: 16, overflow: "hidden", boxShadow: "var(--shadow-lg)", border: "1px solid var(--line)" }}>
            ...
            <div style={{ position: "absolute", top: 16, left: 16 }}><GiftBadge kind={set.badge} /></div>
        ...
        <div>
          ...
          <Reveal delay={240}>
            <div style={{ display: "flex", gap: 12, flexWrap: "wrap" }}>
              ...
              <button onClick={() => gridRef && gridRef.current && gridRef.current.scrollIntoView({ behavior: "smooth", block: "start" })} className="btn btn-ghost" style={{ width: "auto", padding: "15px 26px" }}>See all sets</button>
            </div>
          </Reveal>
        </div>
      </div>
...
/* ======================= GRID (remaining sets + build-your-own) ======================= */
function GiftCard({ set, market }) {
  ...
  return (
    <div id={"set-" + set.id} className="aranya" onMouseEnter={() => setH(true)} onMouseLeave={() => setH(false)}
      style={{ scrollMarginTop: 120, display: "flex", flexDirection: "column", background: "var(--surface)", borderRadius: 12, overflow: "hidden",
        borderTop: `5px solid ${set.color}`, boxShadow: h ? "var(--shadow-md)" : "var(--shadow-sm)", transform: h ? "translateY(-3px)" : "none", transition: "box-shadow .2s, transform .2s" }}>
...
function BuildYourOwn({ market }) {
  const [h, setH] = gUse(false);
  return (
    <a href="Catalog.html" onMouseEnter={() => setH(true)} onMouseLeave={() => setH(false)}
      style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", textAlign: "center", borderRadius: 12,
        border: "1.5px dashed var(--gold-line)", background: h ? "var(--surface)" : "transparent", padding: "40px 28px", minHeight: 260,
        transition: "background .2s, transform .2s", transform: h ? "translateY(-3px)" : "none" }}>
      ...
      <p className="prose" style={{ fontSize: 14.5, color: "var(--muted)", margin: "0 0 18px", maxWidth: 260 }}>Hand-pick any spices from the range and we'll wrap them together as a gift.</p>
...
function GiftGrid({ market, gridRef }) {
  const sets = (window.GIFTS || []).filter((g) => !g.featured);
  return (
    <section ref={gridRef} id="sets" style={{ background: "var(--bg)", padding: "20px 0 100px", scrollMarginTop: 90 }}>
      <div className="home-section-pad" style={{ maxWidth: 1180, margin: "0 auto", padding: "0 40px" }}>
        ...
        <div className="gift-grid" style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 22 }}>
          ...
          <Reveal delay={(sets.length % 3) * 70} style={{ display: "flex" }}><div style={{ flex: 1, display: "flex" }}><BuildYourOwn market={market} /></div></Reveal>
        </div>
      </div>
    </section>
  );
    ...
    <section style={{ background: "var(--bg)", padding: "96px 0" }}>
      <div className="home-section-pad" style={{ maxWidth: 1280, margin: "0 auto", padding: "0 40px" }}>
        <div style={{ display: "flex", alignItems: "flex-end", justifyContent: "space-between", gap: 20, flexWrap: "wrap", marginBottom: 44 }}>
          ...
          <Reveal delay={100}><p style={{ fontFamily: "var(--font-ui)", fontSize: 14.5, color: "var(--muted)", maxWidth: 320, lineHeight: 1.6, paddingBottom: 6 }}>Not sure where to start? We've matched a box to the moment.</p></Reveal>
        </div>
        <div className="gift-occ" style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 18 }}>
          {occ.map((o, i) => <Reveal key={o.name} delay={i * 60}><OccasionTile o={o} /></Reveal>)}
        </div>
      </div>
    </section>
  );
...
function OccasionTile({ o }) {
  ...
  return (
    <a href={"#set-" + o.set} onMouseEnter={() => setH(true)} onMouseLeave={() => setH(false)}
      ...
      <image-slot id={o.slot} shape="rect" fit="cover" placeholder={`Drop a ${o.name} gifting photo`}
        style={{ position: "absolute", inset: 0, width: "100%", height: "100%", display: "block" }}></image-slot>
      ...
      <div className="home-section-pad gift-corp" style={{ maxWidth: 1080, margin: "0 auto", padding: "0 40px", display: "grid", gridTemplateColumns: "1fr auto", gap: 40, alignItems: "center" }}>
        <div>
          ...
          <Reveal delay={100}><p className="prose" style={{ fontSize: 16, color: "var(--muted)", margin: 0, maxWidth: 520 }}>
            We produce custom-branded boxes, bespoke selections and volume orders with your own message card — shipped to one address or many. Tell us the occasion and headcount and we'll build a quote.
          </p></Reveal>
        </div>
        <Reveal delay={120}>
          <div style={{ display: "flex", flexDirection: "column", gap: 12, minWidth: 220 }}>
            <a href="Contact.html" className={btn} style={{ width: "auto", padding: "14px 26px", textAlign: "center" }}>Enquire about corporate gifts</a>
            <a href="Wholesale.html" className="btn btn-ghost" style={{ width: "auto", padding: "14px 26px", textAlign: "center" }}>See wholesale &amp; trade</a>
          </div>
        </Reveal>
      </div>
    </section>
...
Object.assign(window, { GiftHero, FeaturedSet, GiftGrid, FinishingBand, GiftOccasions, GiftCorporate });
