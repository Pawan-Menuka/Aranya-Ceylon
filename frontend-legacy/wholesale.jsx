/* Aranya Ceylon — WHOLESALE / Trade page sections.
   Storefront-styled (aranya.css). Depends on home-common.jsx (Reveal, Liyawel, Eyebrow),
   shared.jsx (Seal), image-slot.js. Application form captures what the admin Wholesale
   queue shows: company, contact, country, business type, monthly volume.
   Exports the section components used by Wholesale.html. */
const { useState: wState, useRef: wRef } = React;

/* ---------------- local icon set (stroke) ---------------- */
function WIcon({ name, size = 22, stroke = "var(--brand)", w = 1.6 }) {
  const p = {
    ...
    scale: <><path d="M12 3v18M7 21h10" /><path d="M5 6h14M5 6l-3 7a3 3 0 0 0 6 0L5 6zM19 6l-3 7a3 3 0 0 0 6 0l-3-7z" /></>,
...
/* ======================= HERO ======================= */
function WholesaleHero({ market, scrollToForm }) {
  ...
  return (
    <header data-hero style={{ position: "relative", minHeight: "92vh", background: "#161412", color: "#FDFAF5", overflow: "hidden", display: "flex", alignItems: "flex-end" }}>
      <image-slot id="wholesale-hero" shape="rect" fit="cover" placeholder="Drop a hero photo — sacks of spice / warehouse / hands with bulk cinnamon"
      ...
      <div className="home-section-pad" style={{ position: "relative", maxWidth: 1280, margin: "0 auto", padding: "0 40px 76px", width: "100%" }}>
        ...
        <Reveal delay={210}>
          <div style={{ display: "flex", gap: 14, marginTop: 34, flexWrap: "wrap" }}>
            ...
            <button className="btn" style={{ width: "auto", padding: "15px 26px", background: "rgba(253,250,245,.12)", color: "#FDFAF5", border: "1px solid rgba(253,250,245,.34)", backdropFilter: "blur(4px)" }}>Request the price list</button>
          </div>
        </Reveal>
        {/* stat strip */}
        <Reveal delay={270}>
    ...
    <section style={{ background: "var(--surface)", padding: "92px 0", borderTop: "1px solid var(--line)", borderBottom: "1px solid var(--line)" }}>
      <div className="home-section-pad" style={{ maxWidth: 1180, margin: "0 auto", padding: "0 40px" }}>
        <div style={{ display: "flex", alignItems: "flex-end", justifyContent: "space-between", gap: 20, flexWrap: "wrap", marginBottom: 44 }}>
          ...
          <Reveal delay={100}><p style={{ fontFamily: "var(--font-ui)", fontSize: 15, color: "var(--muted)", maxWidth: 340, lineHeight: 1.6, paddingBottom: 6 }}>If you sell, cook with, or make food from spice, there's a trade tier for you.</p></Reveal>
        </div>
...
/* ======================= HOW IT WORKS ======================= */
function WholesaleSteps({ scrollToForm, market }) {
  const steps = [
...
/* ======================= VOLUME TIERS ======================= */
function WholesaleTiers({ market, scrollToForm }) {
  const local = market === "local";
  const tiers = [
    {
      name: "Trade Starter", tag: "Cafés, small kitchens & boutiques",
      moq: local ? "5 kg" : "5 kg", priceNote: "List trade pricing",
      save: "Up to 20% off retail",
      features: ["From 5 kg per spice", "Standard retail packaging or bulk", "Card & PayHere / Stripe checkout", "Reorder online anytime"],
      featured: false,
    },
    {
      name: "Trade", tag: "Restaurants, retailers & roasters",
      moq: "25 kg", priceNote: "Volume trade pricing",
      save: "Up to 30% off retail",
      features: ["From 25 kg per spice", "Net-30 terms on approval", "Priority harvest allocation", "Dedicated trade contact", "Quarterly sample boxes"],
      featured: true,
    },
    {
      name: "Distributor & Private Label", tag: "Manufacturers & national accounts",
      moq: "Custom", priceNote: "Contract pricing",
      save: "Custom",
      features: ["Custom volumes & blends", "Your own labelling & pack sizes", "Bespoke logistics & lead times", "Account manager & forecasting"],
      featured: false,
    },
  ];
        ...
        <div className="wh-tier-grid" style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 22, marginTop: 48, alignItems: "stretch" }}>
          {tiers.map((t, i) => (
            <Reveal key={t.name} delay={i * 80} style={{ display: "flex" }}>
              <div style={{ position: "relative", flex: 1, display: "flex", flexDirection: "column", borderRadius: 16, padding: "28px 26px 30px",
                ...
                boxShadow: t.featured ? "0 28px 60px rgba(0,0,0,.4)" : "none" }}>
...
/* ======================= APPLICATION FORM ======================= */
function WholesaleForm({ market, formRef }) {
  const [done, setDone] = wState(false);
  const [ref, setRef] = wState("");
  const [f, setF] = wState({ company: "", contact: "", email: "", phone: "", country: "", type: "", volume: "", website: "", message: "", consent: false });
  const set = (k, v) => setF((x) => ({ ...x, [k]: v }));
  const local = market === "local";
  const btn = local ? "btn btn-local" : "btn btn-intl";
  const volumeBands = local
    ? ["Under Rs 300,000", "Rs 300k – 1.5M", "Rs 1.5M – 4.5M", "Rs 4.5M+"]
  ...
  const submit = (e) => {
    ...
    setDone(true);
    if (formRef && formRef.current) formRef.current.scrollIntoView({ block: "start" });
  };

  const field = (label, key, type = "text", ph = "", req = true) => (
    <label className="wh-field" style={{ display: "flex", flexDirection: "column", gap: 7 }}>
      ...
      <input required={req} type={type} placeholder={ph} value={f[key]} onChange={(e) => set(key, e.target.value)} className="wh-input" />
    </label>
  );
  const select = (label, key, opts, ph) => (
    <label className="wh-field" style={{ display: "flex", flexDirection: "column", gap: 7 }}>
      ...
      <select required value={f[key]} onChange={(e) => set(key, e.target.value)} className="wh-input" style={{ appearance: "none", cursor: "pointer" }}>
  ...
  return (
    <section ref={formRef} id="apply" style={{ background: "var(--bg)", padding: "104px 0", scrollMarginTop: 90 }}>
          ...
          <Reveal delay={190}>
            <div style={{ marginTop: 32, paddingTop: 24, borderTop: "1px solid var(--line)" }}>
              <p style={{ fontFamily: "var(--font-ui)", fontSize: 13.5, color: "var(--muted)", margin: 0, lineHeight: 1.6 }}>
                Prefer to talk? Email <a href="#" style={{ color: "var(--brand)", fontWeight: 700 }}>trade@aranyaceylon.com</a> or call <span style={{ color: "var(--ink)", fontWeight: 600 }}>+94 81 249 0000</span>.
              </p>
            </div>
          </Reveal>
        </div>
          ...
          <div style={{ background: "#fff", border: "1px solid var(--line)", borderRadius: 16, boxShadow: "var(--shadow-md)", padding: "clamp(24px,3vw,40px)" }}>
            {done ? (
              <div style={{ textAlign: "center", padding: "30px 10px" }}>
                ...
                <p style={{ fontFamily: "var(--font-ui)", fontSize: 13.5, color: "var(--muted)", margin: "14px 0 26px" }}>We've emailed a copy to {f.email || "your inbox"}.</p>
                <button className="btn btn-ghost" style={{ width: "auto", padding: "12px 24px", margin: "0 auto" }} onClick={() => { setDone(false); setF({ company: "", contact: "", email: "", phone: "", country: "", type: "", volume: "", website: "", message: "", consent: false }); }}>Submit another</button>
              </div>
            ) : (
              <form onSubmit={submit}>
                ...
                <label style={{ display: "flex", alignItems: "flex-start", gap: 10, marginTop: 18, cursor: "pointer" }}>
                  ...
                  <span style={{ fontFamily: "var(--font-ui)", fontSize: 12.5, color: "var(--muted)", lineHeight: 1.5 }}>I'd like to receive wholesale pricing and harvest updates from Aranya Ceylon. We never share your details.</span>
                </label>
                <button type="submit" className={btn} style={{ marginTop: 24, padding: "15px" }}>Submit application</button>
                <p style={{ fontFamily: "var(--font-ui)", fontSize: 11.5, color: "var(--muted)", textAlign: "center", marginTop: 14 }}>Applications are reviewed by our trade team — this won't create a public account.</p>
              </form>
            )}
          </div>
        </Reveal>
...
Object.assign(window, { WholesaleHero, WholesaleWhy, WholesaleWho, WholesaleSteps, WholesaleTiers, WholesaleForm, WholesaleFAQ });
