/* Aranya Ceylon — homepage shared helpers: scroll-reveal + Liyawel motif */
const { useState: hUseState, useEffect: hUseEffect, useRef: hUseRef } = React;

/* Scroll-reveal: rises + fades as it enters the viewport. Restrained. */
function Reveal({ children, delay = 0, y = 26, as = "div", style = {}, ...rest }) {
  const ref = hUseRef(null);
  const [shown, setShown] = hUseState(false);
  hUseEffect(() => {
    const el = ref.current;
    if (!el) return;
    // Respect reduced-motion, or environments without IO → show immediately.
    ...
    io.observe(el);
    // Failsafe: never let content stay hidden if IO misfires.
...
/* Liyawel — refined geometric creeper-scroll divider (the brand's signature detail).
   Symmetric vine that unfurls from a central lotus bud. Thin gold strokes. */
function Liyawel({ width = 260, color = "var(--gold-line)", accent = "var(--accent)", style = {} }) {
  return (
    <div style={{ display: "flex", justifyContent: "center", ...style }}>
      <svg width={width} height={width * (34 / 260)} viewBox="0 0 260 34" fill="none" role="presentation"
...
function Eyebrow({ children, color = "var(--accent)", center = false, light = false }) {
  const line = light ? "rgba(230,184,96,.6)" : color;
  return (
    <div className="eyebrow" style={{ color: light ? "#E6B860" : color, display: "inline-flex", alignItems: "center", gap: 10,
      justifyContent: center ? "center" : "flex-start" }}>
      <span style={{ width: 24, height: 1, background: line }} />{children}
      {center && <span style={{ width: 24, height: 1, background: line }} />}
    </div>
  );
}

Object.assign(window, { Reveal, Liyawel, Eyebrow });
