/* Aranya Ceylon — Recipes dataset.
   Structured recipes (richer than Journal posts): timings, yield, difficulty, grouped
   ingredients, numbered method, tips, and the catalog spices each recipe features
   (so the detail page can offer "shop the spices"). Plain JS — load before the React files.
   Fields:
     slug, title, dek, course, accent (spice colour), slot (image-slot id), featured
     time {prep, cook}, serves, difficulty, intro
     spices: [CATALOG product names]
     ingredients: [{group?, items:[ "..." ]}]
     method:  [ "step text" ]
     tips:    [ "tip text" ]
*/
window.RECIPE_COURSES = ["All", "Curries & Mains", "Sweet & Bakes", "Drinks", "Sides & Basics"];
...
window.RECIPES = [
  {
    ...
    intro: "Black curry — kalu pol nivithta no, kaḷu — gets its name from the colour of the roasted spice, not the meat. The trick is to dry-roast the whole spices well past golden, then grind and bloom them. Don't rush the roast; it is the entire dish.",
